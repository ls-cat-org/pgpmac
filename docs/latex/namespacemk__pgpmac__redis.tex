\hypertarget{namespacemk__pgpmac__redis}{\section{mk\-\_\-pgpmac\-\_\-redis Namespace Reference}
\label{namespacemk__pgpmac__redis}\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}}
}
\subsection*{Functions}
\begin{DoxyCompactItemize}
\item 
def \hyperlink{namespacemk__pgpmac__redis_ae9b221a54b2fec9e5e3ed2e3c608b39f}{mk\-\_\-home}
\item 
def \hyperlink{namespacemk__pgpmac__redis_a3d3677b3f12b868f4d4a0c0f122e7179}{mk\-\_\-active\-\_\-init}
\item 
def \hyperlink{namespacemk__pgpmac__redis_ac69e5c50c2f30dc7956173aa0e40ae74}{mk\-\_\-inactive\-\_\-init}
\item 
def \hyperlink{namespacemk__pgpmac__redis_a2572732af48007dc8f70e12ed2a738fe}{active\-\_\-simulation}
\item 
def \hyperlink{namespacemk__pgpmac__redis_afe77e7198e6992a85f09dc3667d931d7}{asis}
\end{DoxyCompactItemize}
\subsection*{Variables}
\begin{DoxyCompactItemize}
\item 
list \hyperlink{namespacemk__pgpmac__redis_a760ff4955cbf5beef4ae36a28c30b01a}{head} sys.\-argv\mbox{[}1\mbox{]}
\item 
list \hyperlink{namespacemk__pgpmac__redis_a38c785af5201403976e1267c47ea5096}{pref\-\_\-ini} sys.\-argv\mbox{[}2\mbox{]}
\item 
list \hyperlink{namespacemk__pgpmac__redis_a5864d9c27cbe61534756880cbfebe4f1}{hard\-\_\-ini} sys.\-argv\mbox{[}3\mbox{]}
\item 
dictionary \hyperlink{namespacemk__pgpmac__redis_a9dc9b1bd47efd207604312cd60637526}{configs}
\item 
dictionary \hyperlink{namespacemk__pgpmac__redis_ad8583d4fe88c4c98af73d2858c51c660}{motor\-\_\-dict}
\item 
dictionary \hyperlink{namespacemk__pgpmac__redis_a8257226983aee079ec66f5cc67e194ec}{hard\-\_\-ini\-\_\-fields}
\item 
list \hyperlink{namespacemk__pgpmac__redis_a7228dc1b6ecec376538db1efe8c05ffb}{motor\-\_\-field\-\_\-lists}
\item 
list \hyperlink{namespacemk__pgpmac__redis_a961bfabfbdcbf5b749f6e6ecdc733619}{bi\-\_\-list} \mbox{[}\char`\"{}Cryo\-Switch\char`\"{}\mbox{]}
\item 
dictionary \hyperlink{namespacemk__pgpmac__redis_a2a04d8d0b7270384d1fac674c29e774a}{motor\-\_\-presets}
\item 
list \hyperlink{namespacemk__pgpmac__redis_ad91bac9be746f99e1cf1f8e28ff348be}{zoom\-\_\-settings}
\item 
tuple \hyperlink{namespacemk__pgpmac__redis_af0e472a4df25fc5c1660ef07226689a8}{hi} \hyperlink{classiniParser_1_1iniParser}{ini\-Parser.\-ini\-Parser}( \hyperlink{namespacemk__pgpmac__redis_a5864d9c27cbe61534756880cbfebe4f1}{hard\-\_\-ini})
\item 
list \hyperlink{namespacemk__pgpmac__redis_ac732d49b0afca9a9951e3a252bae42fc}{v} \hyperlink{namespacemk__pgpmac__redis_ad8583d4fe88c4c98af73d2858c51c660}{motor\-\_\-dict}\mbox{[}m\mbox{]}
\item 
string \hyperlink{namespacemk__pgpmac__redis_a057fe7457503e0de97edcf855591ed58}{f} \char`\"{}H\-S\-E\-T\-N\-X\char`\"{}
\item 
list \hyperlink{namespacemk__pgpmac__redis_afba647feb8fc4d0a364c3dca29af3a22}{xlate} \hyperlink{namespacemk__pgpmac__redis_a8257226983aee079ec66f5cc67e194ec}{hard\-\_\-ini\-\_\-fields}\mbox{[}k\mbox{]}
\item 
tuple \hyperlink{namespacemk__pgpmac__redis_a2f5bbda0250eecd94d166dc0a2fbff86}{pi} \hyperlink{classiniParser_1_1iniParser}{ini\-Parser.\-ini\-Parser}( \hyperlink{namespacemk__pgpmac__redis_a38c785af5201403976e1267c47ea5096}{pref\-\_\-ini})
\item 
int \hyperlink{namespacemk__pgpmac__redis_afa643a23a5984fe44c2182ada3dfa401}{i} 0
\item 
tuple \hyperlink{namespacemk__pgpmac__redis_aadd7321b8f7efe139c3e92e51cd34c00}{ppos} pi.\-get( section, option)
\item 
string \hyperlink{namespacemk__pgpmac__redis_a654b54ba0758b8b84516938260851129}{fnc} \char`\"{}H\-S\-E\-T\-N\-X\char`\"{}
\item 
tuple \hyperlink{namespacemk__pgpmac__redis_a90faa2042277e396b48b19374c9db4ab}{b} pi.\-get( section, \char`\"{}Light\-Intensity\char`\"{})
\item 
tuple \hyperlink{namespacemk__pgpmac__redis_a11daf2847f2dc94562b5b61b3f412574}{p} pi.\-get( section, \char`\"{}Motor\-Position\char`\"{})
\item 
tuple \hyperlink{namespacemk__pgpmac__redis_abc8696f6f2f3fd3edd30989d3c8ec301}{x} pi.\-get( section, \char`\"{}Scale\-X\char`\"{})
\item 
tuple \hyperlink{namespacemk__pgpmac__redis_a34220d58136fb95853f54ed51a130f79}{y} pi.\-get( section, \char`\"{}Scale\-Y\char`\"{})
\end{DoxyCompactItemize}


\subsection{Function Documentation}
\hypertarget{namespacemk__pgpmac__redis_a2572732af48007dc8f70e12ed2a738fe}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!active\-\_\-simulation@{active\-\_\-simulation}}
\index{active\-\_\-simulation@{active\-\_\-simulation}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{active\-\_\-simulation}]{\setlength{\rightskip}{0pt plus 5cm}def mk\-\_\-pgpmac\-\_\-redis.\-active\-\_\-simulation (
\begin{DoxyParamCaption}
\item[{}]{sim}
\end{DoxyParamCaption}
)}}\label{namespacemk__pgpmac__redis_a2572732af48007dc8f70e12ed2a738fe}


Definition at line 231 of file mk\-\_\-pgpmac\-\_\-redis.\-py.


\begin{DoxyCode}
231 
232 \textcolor{keyword}{def }\hyperlink{namespacemk__pgpmac__redis_a2572732af48007dc8f70e12ed2a738fe}{active\_simulation}( sim):
233     \textcolor{keywordflow}{if} str(sim) != \textcolor{stringliteral}{"0"}:
234         rtn = \textcolor{stringliteral}{"0"}
235     \textcolor{keywordflow}{else}:
236         rtn = \textcolor{stringliteral}{"1"}
237     \textcolor{keywordflow}{return} rtn

\end{DoxyCode}
\hypertarget{namespacemk__pgpmac__redis_afe77e7198e6992a85f09dc3667d931d7}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!asis@{asis}}
\index{asis@{asis}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{asis}]{\setlength{\rightskip}{0pt plus 5cm}def mk\-\_\-pgpmac\-\_\-redis.\-asis (
\begin{DoxyParamCaption}
\item[{}]{arg}
\end{DoxyParamCaption}
)}}\label{namespacemk__pgpmac__redis_afe77e7198e6992a85f09dc3667d931d7}


Definition at line 238 of file mk\-\_\-pgpmac\-\_\-redis.\-py.


\begin{DoxyCode}
238 
239 \textcolor{keyword}{def }\hyperlink{namespacemk__pgpmac__redis_afe77e7198e6992a85f09dc3667d931d7}{asis}( arg):
240     \textcolor{keywordflow}{return} arg

\end{DoxyCode}
\hypertarget{namespacemk__pgpmac__redis_a3d3677b3f12b868f4d4a0c0f122e7179}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!mk\-\_\-active\-\_\-init@{mk\-\_\-active\-\_\-init}}
\index{mk\-\_\-active\-\_\-init@{mk\-\_\-active\-\_\-init}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{mk\-\_\-active\-\_\-init}]{\setlength{\rightskip}{0pt plus 5cm}def mk\-\_\-pgpmac\-\_\-redis.\-mk\-\_\-active\-\_\-init (
\begin{DoxyParamCaption}
\item[{}]{d}
\end{DoxyParamCaption}
)}}\label{namespacemk__pgpmac__redis_a3d3677b3f12b868f4d4a0c0f122e7179}


Definition at line 209 of file mk\-\_\-pgpmac\-\_\-redis.\-py.


\begin{DoxyCode}
209 
210 \textcolor{keyword}{def }\hyperlink{namespacemk__pgpmac__redis_a3d3677b3f12b868f4d4a0c0f122e7179}{mk\_active\_init}( d):
211     \textcolor{keywordflow}{if} \textcolor{keywordflow}{not} d.has\_key(\textcolor{stringliteral}{"motor\_num"}) \textcolor{keywordflow}{or} \textcolor{keywordflow}{not} d.has\_key(\textcolor{stringliteral}{"coord\_num"}) \textcolor{keywordflow}{or} \textcolor{keywordflow}{not} 
      d.has\_key( \textcolor{stringliteral}{"axis"}):
212         \textcolor{keywordflow}{return} \textcolor{stringliteral}{""}
213     motor\_num = int(d[\textcolor{stringliteral}{"motor\_num"}])
214     coord\_num = int(d[\textcolor{stringliteral}{"coord\_num"}])
215     axis      = str(d[\textcolor{stringliteral}{"axis"}])
216     mask      = 1 << (motor\_num - 1)
217     \textcolor{keywordflow}{if} motor\_num < 1 \textcolor{keywordflow}{or} motor\_num > 32:
218         \textcolor{keywordflow}{return} \textcolor{stringliteral}{""}
219     \textcolor{keywordflow}{return} \textcolor{stringliteral}{'\{M%d=1,&%d#%d->%s,"M700=(M700 | $%0x) ^ $%0x"\}'} % (motor\_num + 30, 
      coord\_num, motor\_num, axis, mask, mask)

\end{DoxyCode}
\hypertarget{namespacemk__pgpmac__redis_ae9b221a54b2fec9e5e3ed2e3c608b39f}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!mk\-\_\-home@{mk\-\_\-home}}
\index{mk\-\_\-home@{mk\-\_\-home}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{mk\-\_\-home}]{\setlength{\rightskip}{0pt plus 5cm}def mk\-\_\-pgpmac\-\_\-redis.\-mk\-\_\-home (
\begin{DoxyParamCaption}
\item[{}]{mname, }
\item[{}]{d}
\end{DoxyParamCaption}
)}}\label{namespacemk__pgpmac__redis_ae9b221a54b2fec9e5e3ed2e3c608b39f}


Definition at line 194 of file mk\-\_\-pgpmac\-\_\-redis.\-py.


\begin{DoxyCode}
194 
195 \textcolor{keyword}{def }\hyperlink{namespacemk__pgpmac__redis_ae9b221a54b2fec9e5e3ed2e3c608b39f}{mk\_home}( mname, d):
196     \textcolor{keywordflow}{if} \textcolor{keywordflow}{not} d.has\_key(\textcolor{stringliteral}{"motor\_num"}) \textcolor{keywordflow}{or} \textcolor{keywordflow}{not} d.has\_key(\textcolor{stringliteral}{"coord\_num"}):
197         \textcolor{keywordflow}{return} \textcolor{stringliteral}{""}
198     motor\_num = int(d[\textcolor{stringliteral}{"motor\_num"}])
199     coord\_num = int(d[\textcolor{stringliteral}{"coord\_num"}])
200     \textcolor{keywordflow}{if} motor\_num < 1 \textcolor{keywordflow}{or} motor\_num > 32:
201         \textcolor{keywordflow}{return} \textcolor{stringliteral}{""}
202     
203     \textcolor{keywordflow}{if} mname == \textcolor{stringliteral}{"kappa"}:
204         prog\_num = 119
205     \textcolor{keywordflow}{else}:
206         prog\_num = motor\_num
207 
208     \textcolor{keywordflow}{return} \textcolor{stringliteral}{'\{#%d$,M%d=1,&%dE,#%d&%dB%dR\}'} % (motor\_num, motor\_num+400, 
      coord\_num, motor\_num, coord\_num, prog\_num)

\end{DoxyCode}
\hypertarget{namespacemk__pgpmac__redis_ac69e5c50c2f30dc7956173aa0e40ae74}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!mk\-\_\-inactive\-\_\-init@{mk\-\_\-inactive\-\_\-init}}
\index{mk\-\_\-inactive\-\_\-init@{mk\-\_\-inactive\-\_\-init}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{mk\-\_\-inactive\-\_\-init}]{\setlength{\rightskip}{0pt plus 5cm}def mk\-\_\-pgpmac\-\_\-redis.\-mk\-\_\-inactive\-\_\-init (
\begin{DoxyParamCaption}
\item[{}]{d}
\end{DoxyParamCaption}
)}}\label{namespacemk__pgpmac__redis_ac69e5c50c2f30dc7956173aa0e40ae74}


Definition at line 220 of file mk\-\_\-pgpmac\-\_\-redis.\-py.


\begin{DoxyCode}
220 
221 \textcolor{keyword}{def }\hyperlink{namespacemk__pgpmac__redis_ac69e5c50c2f30dc7956173aa0e40ae74}{mk\_inactive\_init}( d):
222     \textcolor{keywordflow}{if} \textcolor{keywordflow}{not} d.has\_key(\textcolor{stringliteral}{"motor\_num"}) \textcolor{keywordflow}{or} \textcolor{keywordflow}{not} d.has\_key(\textcolor{stringliteral}{"coord\_num"}) \textcolor{keywordflow}{or} \textcolor{keywordflow}{not} 
      d.has\_key( \textcolor{stringliteral}{"axis"}):
223         \textcolor{keywordflow}{return} \textcolor{stringliteral}{""}
224     motor\_num = int(d[\textcolor{stringliteral}{"motor\_num"}])
225     coord\_num = int(d[\textcolor{stringliteral}{"coord\_num"}])
226     axis      = str(d[\textcolor{stringliteral}{"axis"}])
227     mask      = 1 << (motor\_num - 1)
228     \textcolor{keywordflow}{if} motor\_num < 1 \textcolor{keywordflow}{or} motor\_num > 32:
229         \textcolor{keywordflow}{return} \textcolor{stringliteral}{""}
230     \textcolor{keywordflow}{return} \textcolor{stringliteral}{'\{M%d=0,&%d#%d->0,"M700=M700 | $%0x"\}'} % (motor\_num + 30, coord\_num,
       motor\_num, mask)

\end{DoxyCode}


\subsection{Variable Documentation}
\hypertarget{namespacemk__pgpmac__redis_a90faa2042277e396b48b19374c9db4ab}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!b@{b}}
\index{b@{b}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{b}]{\setlength{\rightskip}{0pt plus 5cm}tuple mk\-\_\-pgpmac\-\_\-redis.\-b pi.\-get( section, \char`\"{}Light\-Intensity\char`\"{})}}\label{namespacemk__pgpmac__redis_a90faa2042277e396b48b19374c9db4ab}


Definition at line 496 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a961bfabfbdcbf5b749f6e6ecdc733619}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!bi\-\_\-list@{bi\-\_\-list}}
\index{bi\-\_\-list@{bi\-\_\-list}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{bi\-\_\-list}]{\setlength{\rightskip}{0pt plus 5cm}list mk\-\_\-pgpmac\-\_\-redis.\-bi\-\_\-list \mbox{[}\char`\"{}Cryo\-Switch\char`\"{}\mbox{]}}}\label{namespacemk__pgpmac__redis_a961bfabfbdcbf5b749f6e6ecdc733619}


Definition at line 302 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a9dc9b1bd47efd207604312cd60637526}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!configs@{configs}}
\index{configs@{configs}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{configs}]{\setlength{\rightskip}{0pt plus 5cm}dictionary mk\-\_\-pgpmac\-\_\-redis.\-configs}}\label{namespacemk__pgpmac__redis_a9dc9b1bd47efd207604312cd60637526}
{\bfseries Initial value\-:}
\begin{DoxyCode}
1 \{
2     \textcolor{stringliteral}{"orange-2"}            : \{ \textcolor{stringliteral}{"re"} : \textcolor{stringliteral}{"redis\(\backslash\).kvseq|stns\(\backslash\).2\(\backslash\).(.+)"}, \textcolor{stringliteral}{"head"} : \textcolor{stringliteral}{"
      stns.2"}, \textcolor{stringliteral}{"pub"} : \textcolor{stringliteral}{"MD2-21-ID-E"}\},
3     \textcolor{stringliteral}{"orange-2.ls-cat.org"} : \{ \textcolor{stringliteral}{"re"} : \textcolor{stringliteral}{"redis\(\backslash\).kvseq|stns\(\backslash\).2\(\backslash\).(.+)"}, \textcolor{stringliteral}{"head"} : \textcolor{stringliteral}{"
      stns.2"}, \textcolor{stringliteral}{"pub"} : \textcolor{stringliteral}{"MD2-21-ID-E"}\},
4     \textcolor{stringliteral}{"venison.ls-cat.org"}  : \{ \textcolor{stringliteral}{"re"} : \textcolor{stringliteral}{"redis\(\backslash\).kvseq|stns\(\backslash\).2\(\backslash\).(.+)"}, \textcolor{stringliteral}{"head"} : \textcolor{stringliteral}{"
      stns.2"}, \textcolor{stringliteral}{"pub"} : \textcolor{stringliteral}{"MD2-21-ID-E"}\}
5 \}
\end{DoxyCode}


Definition at line 26 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a057fe7457503e0de97edcf855591ed58}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!f@{f}}
\index{f@{f}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{f}]{\setlength{\rightskip}{0pt plus 5cm}tuple mk\-\_\-pgpmac\-\_\-redis.\-f \char`\"{}H\-S\-E\-T\-N\-X\char`\"{}}}\label{namespacemk__pgpmac__redis_a057fe7457503e0de97edcf855591ed58}


Definition at line 437 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a654b54ba0758b8b84516938260851129}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!fnc@{fnc}}
\index{fnc@{fnc}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{fnc}]{\setlength{\rightskip}{0pt plus 5cm}string mk\-\_\-pgpmac\-\_\-redis.\-fnc \char`\"{}H\-S\-E\-T\-N\-X\char`\"{}}}\label{namespacemk__pgpmac__redis_a654b54ba0758b8b84516938260851129}


Definition at line 487 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a5864d9c27cbe61534756880cbfebe4f1}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!hard\-\_\-ini@{hard\-\_\-ini}}
\index{hard\-\_\-ini@{hard\-\_\-ini}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{hard\-\_\-ini}]{\setlength{\rightskip}{0pt plus 5cm}mk\-\_\-pgpmac\-\_\-redis.\-hard\-\_\-ini sys.\-argv\mbox{[}3\mbox{]}}}\label{namespacemk__pgpmac__redis_a5864d9c27cbe61534756880cbfebe4f1}


Definition at line 21 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a8257226983aee079ec66f5cc67e194ec}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!hard\-\_\-ini\-\_\-fields@{hard\-\_\-ini\-\_\-fields}}
\index{hard\-\_\-ini\-\_\-fields@{hard\-\_\-ini\-\_\-fields}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{hard\-\_\-ini\-\_\-fields}]{\setlength{\rightskip}{0pt plus 5cm}dictionary mk\-\_\-pgpmac\-\_\-redis.\-hard\-\_\-ini\-\_\-fields}}\label{namespacemk__pgpmac__redis_a8257226983aee079ec66f5cc67e194ec}
{\bfseries Initial value\-:}
\begin{DoxyCode}
1 \{
2     \textcolor{stringliteral}{"active"}          : [\textcolor{stringliteral}{"Simulation"}, active\_simulation],
3     \textcolor{stringliteral}{"coord\_num"}       : [\textcolor{stringliteral}{"CoordinateSystem"}, asis],
4     \textcolor{stringliteral}{"largeStep"}       : [\textcolor{stringliteral}{"LargeStep"}, asis],
5     \textcolor{stringliteral}{"maxPosition"}     : [\textcolor{stringliteral}{"MaxPosition"}, asis],
6     \textcolor{stringliteral}{"minPosition"}     : [\textcolor{stringliteral}{"MinPosition"}, asis],
7     \textcolor{stringliteral}{"motor\_num"}       : [\textcolor{stringliteral}{"MotorNumber"}, asis],
8     \textcolor{stringliteral}{"neutralPosition"} : [\textcolor{stringliteral}{"NeutralPosition"}, asis],
9     \textcolor{stringliteral}{"precision"}       : [\textcolor{stringliteral}{"Precision"}, asis],
10     \textcolor{stringliteral}{"smallStep"}       : [\textcolor{stringliteral}{"SmallStep"}, asis],
11     \textcolor{stringliteral}{"u2c"}             : [\textcolor{stringliteral}{"UnitRatio"}, asis]
12     \}
\end{DoxyCode}


Definition at line 241 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a760ff4955cbf5beef4ae36a28c30b01a}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!head@{head}}
\index{head@{head}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{head}]{\setlength{\rightskip}{0pt plus 5cm}list mk\-\_\-pgpmac\-\_\-redis.\-head sys.\-argv\mbox{[}1\mbox{]}}}\label{namespacemk__pgpmac__redis_a760ff4955cbf5beef4ae36a28c30b01a}


Definition at line 13 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_af0e472a4df25fc5c1660ef07226689a8}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!hi@{hi}}
\index{hi@{hi}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{hi}]{\setlength{\rightskip}{0pt plus 5cm}tuple mk\-\_\-pgpmac\-\_\-redis.\-hi {\bf ini\-Parser.\-ini\-Parser}( {\bf hard\-\_\-ini})}}\label{namespacemk__pgpmac__redis_af0e472a4df25fc5c1660ef07226689a8}


Definition at line 393 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_afa643a23a5984fe44c2182ada3dfa401}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!i@{i}}
\index{i@{i}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{i}]{\setlength{\rightskip}{0pt plus 5cm}int mk\-\_\-pgpmac\-\_\-redis.\-i 0}}\label{namespacemk__pgpmac__redis_afa643a23a5984fe44c2182ada3dfa401}


Definition at line 461 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_ad8583d4fe88c4c98af73d2858c51c660}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!motor\-\_\-dict@{motor\-\_\-dict}}
\index{motor\-\_\-dict@{motor\-\_\-dict}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{motor\-\_\-dict}]{\setlength{\rightskip}{0pt plus 5cm}dictionary mk\-\_\-pgpmac\-\_\-redis.\-motor\-\_\-dict}}\label{namespacemk__pgpmac__redis_ad8583d4fe88c4c98af73d2858c51c660}


Definition at line 64 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a7228dc1b6ecec376538db1efe8c05ffb}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!motor\-\_\-field\-\_\-lists@{motor\-\_\-field\-\_\-lists}}
\index{motor\-\_\-field\-\_\-lists@{motor\-\_\-field\-\_\-lists}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{motor\-\_\-field\-\_\-lists}]{\setlength{\rightskip}{0pt plus 5cm}list mk\-\_\-pgpmac\-\_\-redis.\-motor\-\_\-field\-\_\-lists}}\label{namespacemk__pgpmac__redis_a7228dc1b6ecec376538db1efe8c05ffb}


Definition at line 263 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a2a04d8d0b7270384d1fac674c29e774a}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!motor\-\_\-presets@{motor\-\_\-presets}}
\index{motor\-\_\-presets@{motor\-\_\-presets}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{motor\-\_\-presets}]{\setlength{\rightskip}{0pt plus 5cm}dictionary mk\-\_\-pgpmac\-\_\-redis.\-motor\-\_\-presets}}\label{namespacemk__pgpmac__redis_a2a04d8d0b7270384d1fac674c29e774a}


Definition at line 304 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a11daf2847f2dc94562b5b61b3f412574}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!p@{p}}
\index{p@{p}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{p}]{\setlength{\rightskip}{0pt plus 5cm}tuple mk\-\_\-pgpmac\-\_\-redis.\-p pi.\-get( section, \char`\"{}Motor\-Position\char`\"{})}}\label{namespacemk__pgpmac__redis_a11daf2847f2dc94562b5b61b3f412574}


Definition at line 503 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a2f5bbda0250eecd94d166dc0a2fbff86}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!pi@{pi}}
\index{pi@{pi}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{pi}]{\setlength{\rightskip}{0pt plus 5cm}tuple mk\-\_\-pgpmac\-\_\-redis.\-pi {\bf ini\-Parser.\-ini\-Parser}( {\bf pref\-\_\-ini})}}\label{namespacemk__pgpmac__redis_a2f5bbda0250eecd94d166dc0a2fbff86}


Definition at line 458 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_aadd7321b8f7efe139c3e92e51cd34c00}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!ppos@{ppos}}
\index{ppos@{ppos}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{ppos}]{\setlength{\rightskip}{0pt plus 5cm}tuple mk\-\_\-pgpmac\-\_\-redis.\-ppos pi.\-get( section, option)}}\label{namespacemk__pgpmac__redis_aadd7321b8f7efe139c3e92e51cd34c00}


Definition at line 469 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a38c785af5201403976e1267c47ea5096}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!pref\-\_\-ini@{pref\-\_\-ini}}
\index{pref\-\_\-ini@{pref\-\_\-ini}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{pref\-\_\-ini}]{\setlength{\rightskip}{0pt plus 5cm}mk\-\_\-pgpmac\-\_\-redis.\-pref\-\_\-ini sys.\-argv\mbox{[}2\mbox{]}}}\label{namespacemk__pgpmac__redis_a38c785af5201403976e1267c47ea5096}


Definition at line 16 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_ac732d49b0afca9a9951e3a252bae42fc}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!v@{v}}
\index{v@{v}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{v}]{\setlength{\rightskip}{0pt plus 5cm}tuple mk\-\_\-pgpmac\-\_\-redis.\-v {\bf motor\-\_\-dict}\mbox{[}m\mbox{]}}}\label{namespacemk__pgpmac__redis_ac732d49b0afca9a9951e3a252bae42fc}


Definition at line 436 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_abc8696f6f2f3fd3edd30989d3c8ec301}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!x@{x}}
\index{x@{x}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{x}]{\setlength{\rightskip}{0pt plus 5cm}tuple mk\-\_\-pgpmac\-\_\-redis.\-x pi.\-get( section, \char`\"{}Scale\-X\char`\"{})}}\label{namespacemk__pgpmac__redis_abc8696f6f2f3fd3edd30989d3c8ec301}


Definition at line 510 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_afba647feb8fc4d0a364c3dca29af3a22}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!xlate@{xlate}}
\index{xlate@{xlate}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{xlate}]{\setlength{\rightskip}{0pt plus 5cm}list mk\-\_\-pgpmac\-\_\-redis.\-xlate {\bf hard\-\_\-ini\-\_\-fields}\mbox{[}k\mbox{]}}}\label{namespacemk__pgpmac__redis_afba647feb8fc4d0a364c3dca29af3a22}


Definition at line 441 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_a34220d58136fb95853f54ed51a130f79}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!y@{y}}
\index{y@{y}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{y}]{\setlength{\rightskip}{0pt plus 5cm}tuple mk\-\_\-pgpmac\-\_\-redis.\-y pi.\-get( section, \char`\"{}Scale\-Y\char`\"{})}}\label{namespacemk__pgpmac__redis_a34220d58136fb95853f54ed51a130f79}


Definition at line 517 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

\hypertarget{namespacemk__pgpmac__redis_ad91bac9be746f99e1cf1f8e28ff348be}{\index{mk\-\_\-pgpmac\-\_\-redis@{mk\-\_\-pgpmac\-\_\-redis}!zoom\-\_\-settings@{zoom\-\_\-settings}}
\index{zoom\-\_\-settings@{zoom\-\_\-settings}!mk_pgpmac_redis@{mk\-\_\-pgpmac\-\_\-redis}}
\subsubsection[{zoom\-\_\-settings}]{\setlength{\rightskip}{0pt plus 5cm}list mk\-\_\-pgpmac\-\_\-redis.\-zoom\-\_\-settings}}\label{namespacemk__pgpmac__redis_ad91bac9be746f99e1cf1f8e28ff348be}
{\bfseries Initial value\-:}
\begin{DoxyCode}
1 [
2     \textcolor{comment}{#lev   front  back  pos     scalex  scaley   section}
3     [1,     4.0,   8.0,  34100, 2.7083,  3.3442, \textcolor{stringliteral}{"CoaxCam.Zoom1"}],
4     [2,     6.0,   8.1,  31440, 2.2487,  2.2776, \textcolor{stringliteral}{"CoaxCam.Zoom2"}],
5     [3,     6.5,   8.2,  27460, 1.7520,  1.7550, \textcolor{stringliteral}{"CoaxCam.Zoom3"}],
6     [4,     7.0,   8.3,  23480, 1.3360,  1.3400, \textcolor{stringliteral}{"CoaxCam.Zoom4"}],
7     [5,     8.0,  10.0,  19500, 1.0140,  1.0110, \textcolor{stringliteral}{"CoaxCam.Zoom5"}],
8     [6,     9.0,  12.0,  15520, 0.7710,  0.7760, \textcolor{stringliteral}{"CoaxCam.Zoom6"}],
9     [7,    10.0,  17.0,  11540, 0.5880,  0.5920, \textcolor{stringliteral}{"CoaxCam.Zoom7"}],
10     [8,    12.0,  25.0,   7560, 0.4460,  0.4480, \textcolor{stringliteral}{"CoaxCam.Zoom8"}],
11     [9,    15.0,  37.0,   3580, 0.3410,  0.3460, \textcolor{stringliteral}{"CoaxCam.Zoom9"}],
12     [10,   16.0,  42.0,      0, 0.2700,  0.2690, \textcolor{stringliteral}{"CoaxCam.Zoom10"}]
13     ]
\end{DoxyCode}


Definition at line 371 of file mk\-\_\-pgpmac\-\_\-redis.\-py.

