...
//inih stuff
  typedef struct {
    const char* maatel_motor_name;
  } inih_config;

  static int station_no;
  char redis_mot_name[100];
  char redis_ostr[1024];
  char redis_fostr[1024];

 //redis stuff
  int no_elements;
  redisReply *redis_reply;
  FILE *kvp_file,*hl_file;
  char out_line[200];
  char in_line[200];
  ...
  void construc_redis_ostr (char ostr, char fostr);
  int parse_microdiff_hard (const char* ini_file, int station_no);
  int parse_micordiff_pref (const char* ini_file, int station_no);
