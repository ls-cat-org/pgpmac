...
int main(
 ...
 //configuration file ... capel 
  FILE *conf_file;
  char sta_no_str[50], station_str[50], redis_ip_str[50];
  char ini_fname[100], kvp_fname[100];
  char conf_comm_str[100], kv_pair_str[100], hash_str[100];
  char new_kvp_file[100],key_spec[100];
...
// read in station-specific configuration
  conf_file = fopen("pgpmac.conf", "r");
     fgets(in_line, 200, conf_file); // station index
     sscanf(in_line,"%d%s",&station_no,&conf_comm_str); 
     printf("station no.:%d\n",station_no);
     
     fgets(in_line, 200, conf_file);   // station config subdir
     sscanf(in_line,"%s%s",&station_str,&conf_comm_str);
     printf("station subdir:%s\n",station_str);
     
     fgets(in_line, 200, conf_file);   // redis ip string
     sscanf(in_line,"%s%s",&redis_ip_str,&conf_comm_str);
     printf("redis ip:%s\n",redis_ip_str);
     
     fgets(in_line, 200, conf_file); //kvpair file spec
     sscanf(in_line,"%s%s",&kv_pair_str,&conf_comm_str);
     printf("kv pair file:%s\n",kv_pair_str); 
  
  fclose(conf_file);

// connect to redis server
  struct timeval timeout = { 1, 500000 }; // 1.5 seconds
  redis_con = redisConnectWithTimeout((char*)redis_ip_str, 6379, timeout);
  printf ("connected to redis server on %s\n",redis_ip_str);
  
  //parse microdiff_hard.ini ... update Redis data base, using the same KVP names Keith used (as much as possible).
  //parsing accomplished via inih package
  sprintf(ini_fname,"./%s/microdiff_hard.ini",station_str);
  if (parse_microdiff_hard((const char *)ini_fname, station_no) < 0 ) {
     printf ("can't load ./%s/microdiff_hard.ini\n",station_str);
     sleep(10);
  }

  //parse microdiff_pref.ini ... update Redis data bse, useing same KVP names Keith used
  sprintf(ini_fname,"./%s/microdiff_pref.ini",station_str);
  if (parse_microdiff_pref((const char *)ini_fname, station_no) < 0) { 
     printf ("can't load ./%s/microdiff_pref.ini\n",station_str);
     sleep(10);
  }
  printf ("ini file parsing compelete\n");
  sleep(1);

  //parse kv pair dump file and commit to redis kvdb
  sprintf(kvp_fname,"./%s/%s",station_str,kv_pair_str);
  if (parse_kv_pairs((char *)kvp_fname) < 0) {
     printf ("can't load kv pair file: %s\b",kv_pair_str);
     sleep(10);
   }
  printf ("kvp file parsing compelete\n");
  sleep(1);

  //make a backup of station kv pair data from redis kv store
  sprintf(new_kvp_file,"%s","./new_kvp_backup.txt");
  sprintf(key_spec,"%s","mdhard.1.");
  save_kvpair_vals(new_kvp_file, key_spec);
 ...
 //start md2 rpc thread  ... capel
  static pthread_t md2_rpc_thread; //md2 rpc thread
  extern void *md2_server_proc( void * dummy );
  pthread_create( &md2_rpc_thread, NULL, &md2_server_proc, NULL);
  lslogging_log_message("RPC thread launched\n");
  ...
  lstimer_init();
  lstimer_run();
  lsredis_init( "MD2-24-ID-C", "redis\\.kvseq|stns\\.1\\.(.+)", "stns.1");
  lsredis_run();
